  ...
  config = {
    ...
    systemd.services.setup-netns-vpn = {
      ...
      serviceConfig = {
        RemainAfterExit = true;
        Type = "oneshot";
        LoadCredential = "media-vpn-wg.conf:${config.age.secrets.vpn-wg.path}";
      };
      path = with pkgs; [
        iproute2
        wireguard-tools
      ...
      script = ''
        ...
        ip netns exec vpn wg syncconf vpn <(wg-quick strip "$CREDENTIALS_DIRECTORY/media-vpn-wg.conf")
        ip -n vpn link set vpn up
        ip -n vpn route add default dev vpn
      '';
    };

    age.secrets.vpn-wg.file = "${self}/secrets/media/media-vpn-wg.age";
